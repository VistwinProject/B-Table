...
| **情境 `scene`** | 顯示選到的角色(如孕婦「極致純淨」),環保留柔白光波,提示換下一個 | 刷到角色鑰匙圈後 |

感應區以常駐的細緻光波引導放置，刷卡後直接切換文字與角色背景，不再顯示打勾或成功漣漪。拿起鑰匙圈會回到「選角色」狀態,方便連續體驗不同角色。
...
感應區位置固定在桌面下方的 NFC reader 正上方；常駐光波不作為刷卡成功提示。邀請卡使用透明卡面與循環邊緣高光。五種角色沿用 B-TV 色票，背景素材 `public/backgrounds/exhibition-arcs.svg` 取自 B2.0ver 的同名 A 字光束，以角色色調呈現並交叉淡入。

### 與 B-TV 2.0 共用感應

目前搭配 `../app/` 的 B-TV 2.0（5284）。請啟動上層 `../server/index.cjs` 作為唯一的 8788 共用 bridge，並分別啟動 TV 與本專案的 Vite。此 bridge 接收桌面下的 USB NFC reader，將同一張角色卡廣播給 table 和 TV；C / 1–5 的模擬也走同一路徑。TV 預設已啟用連線。詳細啟動方式見上層 README 的「Table → TV 連動」。使用此方式時不要同時啟動本資料夾舊的 Python server 或啟動.bat，以免搶占 8788。
